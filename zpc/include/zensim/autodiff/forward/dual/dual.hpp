...
#include <zensim/autodiff/common/numbertraits.hpp>
...
namespace autodiff {
    namespace detail {
        ...
        struct Dual
        {
            ...
            constexpr Dual() : Dual(0.0) {}

            template<typename U, EnableIf<isConvertible<U, T> && !isExpr<U>>* = nullptr>
            //template<typename U, EnableIf<isConvertible<U, T> && !isExpr<U>>* = nullptr>
            //template <typename U, std::enable_if_t<isConvertible<U, T> && !isExpr<U>, int> = 0>
            constexpr Dual(U&& v)
                    : val(FWD(v)), grad(0)
            {
            }
            ...
            template <typename U, EnableIf<isConvertible<U, T> && !isExpr<U>>* = nullptr>
            constexpr Dual(const zs::tuple<U>& o) : Dual{zs::get<0>(o)} {}

            template <typename U, EnableIf<isDual<U>>* = nullptr>
            constexpr Dual(const zs::tuple<U>& o) : Dual{zs::get<0>(o)} {}

            constexpr Dual(const NumericType<T>& val)
                    : val(val), grad(0)
            {
            }

            template<typename U, EnableIf<isExpr<U> && !isDual<U>>* = nullptr>
            static constexpr auto def(U&& other) {
              Dual ret{};
              assign(ret, FWD(other));
              return ret;
            }

            template<typename U, EnableIf<isExpr<U> && !isDual<U>>* = nullptr>
            //template<typename U, EnableIf<isExpr<U> && !isDual<U>>* = nullptr>
            //template <typename U, std::enable_if_t<isExpr<U> && !isDual<U>, int> = 0>
            constexpr Dual(U&& other)
            {

                assign(*this, FWD(other));
            }

            template<typename U, EnableIf<isExpr<U> && !isDual<U>>* = nullptr>
            constexpr Dual& operator=(U&& other)
            {
                Dual tmp{};
                assign(tmp, FWD(other));
                assign(*this, tmp);
                return *this;
            }


            template<typename U, EnableIf<isArithmetic<U> || isExpr<U>>* = nullptr>
            constexpr Dual& operator+=(U&& other)
            {
                Dual tmp{};
                assign(tmp, FWD(other));
      ...
      operator U() const { return static_cast<U>(val); }
        ...
        template<typename Op, typename R>
        constexpr auto inner(const UnaryExpr<Op, R>& expr) -> const R
        {
            return expr.r;
        }

        template<typename Op, typename L, typename R>
        constexpr auto left(const BinaryExpr<Op, L, R>& expr) -> const L
        {
            return expr.l;
        }

        template<typename Op, typename L, typename R>
        constexpr auto right(const BinaryExpr<Op, L, R>& expr) -> const R
        {
            return expr.r;
        }
        ...
        template<typename T>
        constexpr auto eval(T&& expr)
        {
            static_assert(isDual<T> || isExpr<T> || isArithmetic<T>);
            if constexpr (isDual<T>)
                return FWD(expr);
            else if constexpr (isExpr<T>)
                return DualType<T>(FWD(expr));
            else return FWD(expr);
        }

        template<typename T>
        constexpr auto val(T&& expr)
        {
            static_assert(isDual<T> || isExpr<T> || isArithmetic<T>);
            if constexpr (isDual<T>)
                return val(expr.val);
            else if constexpr (isExpr<T>)
                return val(eval(FWD(expr)));
            else return FWD(expr);
        }
        ...
        constexpr auto derivative(const Dual<T, G>& dual)
        {
        ...
        constexpr auto& gradnode(Dual<T, G>& dual)
        {
        ...
        constexpr auto seed(Dual<T, G>& dual, U&& seedval)
        {
        ...
        // POSITIVE OPERATOR: +x
        //-----------------------------------------------------------------------------
        template<typename R, EnableIf<isExpr<R>>* = nullptr>
        constexpr auto operator+(R&& expr)
        {
            return FWD(expr); // expression optimization: +(expr) => expr
        }
        ...
        template<typename L, typename R, EnableIf<isOperable<L, R>>* = nullptr>
        ...
        constexpr auto operator/(L&& l, R&& r)
        {
            if constexpr (isArithmetic<R>)
                return FWD(l) * (One<L>() / FWD(r));
            else return FWD(l) * inverse(FWD(r));
        }
        ...
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto sin(R&& r) -> SinExpr<R> { return { r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto cos(R&& r) -> CosExpr<R> { return { r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto tan(R&& r) -> TanExpr<R> { return { r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto asin(R&& r) -> ArcSinExpr<R> { return { r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto acos(R&& r) -> ArcCosExpr<R> { return { r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto atan(R&& r) -> ArcTanExpr<R> { return { r }; }
        template<typename L, typename R, EnableIf<isOperable<L, R>>* = nullptr> constexpr auto atan2(L&& l, R&& r) -> ArcTan2Expr<L, R> { return { l, r }; }
        template<typename L, typename R, EnableIf<isOperable<L, R>>* = nullptr> constexpr auto hypot(L&& l, R&& r) -> Hypot2Expr<L, R> { return { l, r }; }
        template<typename L, typename C, typename R, EnableIf<isOperable3<L, C, R>>* = nullptr>
        ...
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto sinh(R&& r) -> SinhExpr<R> { return { r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto cosh(R&& r) -> CoshExpr<R> { return { r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto tanh(R&& r) -> TanhExpr<R> { return { r }; }
        ...
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto exp(R&& r) -> ExpExpr<R> { return { r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto log(R&& r) -> LogExpr<R> { return { r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto log10(R&& r) -> Log10Expr<R> { return { r }; }
        ...
        template<typename L, typename R, EnableIf<isOperable<L, R>>* = nullptr> constexpr auto pow(L&& l, R&& r) -> PowExpr<L, R> { return { l, r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto sqrt(R&& r) -> SqrtExpr<R> { return { r }; }
        ...
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto abs(R&& r) -> AbsExpr<R> { return { r }; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto abs2(R&& r) { return FWD(r) * FWD(r); }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto conj(R&& r) { return FWD(r); }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto real(R&& r) { return FWD(r); }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto imag(R&&) { return 0.0; }
        template<typename R, EnableIf<isExpr<R>>* = nullptr> constexpr auto erf(R&& r) -> ErfExpr<R> { return { r }; }
        ...
        template<typename L, typename R, EnableIf<isOperable<L, R>>* = nullptr> constexpr bool operator==(L&& l, R&& r) { return val(l) == val(r); }
        template<typename L, typename R, EnableIf<isOperable<L, R>>* = nullptr> constexpr bool operator!=(L&& l, R&& r) { return val(l) != val(r); }
        template<typename L, typename R, EnableIf<isOperable<L, R>>* = nullptr> constexpr bool operator<=(L&& l, R&& r) { return val(l) <= val(r); }
        template<typename L, typename R, EnableIf<isOperable<L, R>>* = nullptr> constexpr bool operator>=(L&& l, R&& r) { return val(l) >= val(r); }
        template<typename L, typename R, EnableIf<isOperable<L, R>>* = nullptr> constexpr bool operator<(L&& l, R&& r) { return val(l) < val(r); }
        ...
        constexpr void assignArcTan2(Dual<T, G>& self, Y&&y, X&&x)
        {
            ...
            if constexpr (isArithmetic<Y> && isDual<X>) {
                ...
                Dual<T, G> y_tmp{};
                assign(y_tmp, FWD(y));
                assignArcTan2(self, std::move(y_tmp), FWD(x));
            }
                ...
                Dual<T, G> x_tmp{};
                assign(x_tmp, FWD(x));
                assignArcTan2(self, FWD(y), std::move(x_tmp));
            }
        }
        ...
        constexpr void assignHypot2(Dual<T, G>& self, X&& x, Y&& y)
        {
            ...
            if constexpr (isDual<X> && isArithmetic<Y>) {
                ...
                Dual<T, G> x_tmp{};
                assign(x_tmp, FWD(x));
                assignHypot2(self, std::move(x_tmp), FWD(y));
            }
                ...
                Dual<T, G> y_tmp{};
                assign(y_tmp, FWD(y));
                assignHypot2(self, FWD(x), std::move(y_tmp));
            }
        }
        ...
        constexpr void assignHypot3(Dual<T, G>& self, X&& x, Y&& y, Z&& z)
        {
            ...
            if constexpr (isDual<X> && isArithmetic<Y> && isArithmetic<Z>) {
                ...
                Dual<T, G> tmp{};
                assign(tmp, FWD(x));
                assignHypot3(self, std::move(tmp), FWD(y), FWD(z));
            }
                ...
                Dual<T, G> tmp{};
                assign(tmp, FWD(y));
                assignHypot3(self, FWD(x), std::move(tmp), FWD(z));
            }
                ...
                Dual<T, G> tmp{};
                assign(tmp, FWD(z));
                assignHypot3(self, FWD(x), FWD(y), std::move(tmp));
            }
        }
        ...
        template<typename T, EnableIf<isArithmetic<T>>* = nullptr>
