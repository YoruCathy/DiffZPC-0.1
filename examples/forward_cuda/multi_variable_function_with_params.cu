...
constexpr dual f(dual x, const Params& params)
{
  return dual::def(params.a * sin(x) + params.b * cos(x) + params.c * sin(x)*cos(x));
}
...
int main()
{
  ...
  Vector<Params> param{1, memsrc_e::device, 0};
  ...
  cudaPol(enumerate(param), [] __device__(auto id, auto &v) mutable {
    v.a = 1.0;
    v.b = 2.0;
    v.c = 3.0;
    //    printf("v: %f", (float)v);
  ...
  cudaPol(zip(val,param), [] __device__ (auto &val, auto &param) mutable {
    dual x = val;
